...
from orgline.hub.utils import HUB_API_ROOT, HUB_WEB_ROOT, PREFIX, smart_request
from orgline.utils import LOGGER, __version__, checks, emojis, is_colab, threaded
from orgline.utils.errors import HUBModelError
...
class HUBTrainingSession:
    """
    HUB training session for orgline HUB ORGDET models. Handles model initialization, heartbeats, and checkpointing.
    ...
    Attributes:
        ...
        model_id (str): Identifier for the ORGDET model being trained.
        model_url (str): URL for the model in orgline HUB.
        api_url (str): API URL for the model in orgline HUB.
        auth_header (dict): Authentication header for the orgline HUB API requests.
        rate_limits (dict): Rate limits for different API calls (in seconds).
        timers (dict): Timers for rate limiting.
        metrics_queue (dict): Queue for the model's metrics.
        model (dict): Model data fetched from orgline HUB.
    ...
    def __init__(self, url):
        ...
        from orgline.hub.auth import Auth
    ...
    def _get_model(self):
        ...
        try:
            ...
            if data['status'] == 'new':  # new model to start training
                ...
                self.model_file = checks.check_ORGDETv5u_filename(self.model_file, verbose=False)  # ORGDETv5->ORGDETv5u
    ...
    def upload_model(self, epoch, weights, is_best=False, map=0.0, final=False):
        """
        Upload a model checkpoint to orgline HUB.
    ...
    def _start_heartbeat(self):
        """Begin a threaded heartbeat loop to report the agent's status to orgline HUB."""
        while self.alive:
