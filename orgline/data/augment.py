...
from orgline.utils import LOGGER, colorstr
from orgline.utils.checks import check_version
from orgline.utils.instance import Instances
from orgline.utils.metrics import bbox_ioa
from orgline.utils.ops import segment2box
...
class BaseMixTransform:
    ...
    This implementation is from mmORGDET.
    """
...
class Albumentations:
    ...
    def __init__(self, p=1.0):
        """Initialize the transform object for ORGDET bbox formatted params."""
        self.p = p
        self.transform = None
...
class Format:
    ...
    def _format_img(self, img):
        """Format the image for ORGDET from Numpy array to PyTorch tensor."""
        if len(img.shape) < 3:
...
def v8_transforms(dataset, imgsz, hyp, stretch=False):
    """Convert images to a size suitable for ORGDETv8 training."""
...
def classify_albumentations(
    ...
    """ORGDETv8 classification Albumentations (optional, only used if package is installed)."""
    prefix = colorstr('albumentations: ')
    try:
        import albumentations as A
...
class ClassifyLetterBox:
    """
    ORGDETv8 LetterBox class for image preprocessing, designed to be part of a transformation pipeline, e.g.,
...
class CenterCrop:
    """ORGDETv8 CenterCrop class for image preprocessing, designed to be part of a transformation pipeline, e.g.,
...
class ToTensor:
    """ORGDETv8 ToTensor class for image preprocessing, i.e., T.Compose([LetterBox(size), ToTensor()])."""

    def __init__(self, half=False):
        """Initialize ORGDETv8 ToTensor object with optional half-precision support."""
        super().__init__()
        self.half = half
