...
#
# Usage:
#     $ ORGDET mode=train model=ORGDETv8n.pt data=coco128.yaml imgsz=640 epochs=100 batch=16
# """
#
import math
import os
...
from orgline.cfg import get_cfg, get_save_dir
from orgline.data.utils import check_cls_dataset, check_det_dataset
from orgline.nn.tasks import attempt_load_one_weight, attempt_load_weights
from orgline.utils import (DEFAULT_CFG, LOGGER, RANK, TQDM, __version__, callbacks, clean_url, colorstr, emojis,
                               yaml_save)
from orgline.utils.autobatch import check_train_batch_size
from orgline.utils.checks import check_amp, check_file, check_imgsz, print_args
from orgline.utils.dist import ddp_cleanup, generate_ddp_command
from orgline.utils.files import get_latest_run
from orgline.utils.torch_utils import (EarlyStopping, ModelEMA, de_parallel, init_seeds, one_cycle, select_device,
                                           strip_optimizer)
from orgline.nn.extra_modules.kernel_warehouse import get_temperature
...
class BaseTrainer:
        ...
        try:
            if self.args.task == 'classify':
                ...
                if 'yaml_file' in self.data:
                    self.args.data = self.data['yaml_file']  # for validating 'ORGDET train data=url.zip' usage
    ...
    def _setup_train(self, world_size):
        ...
        for k, v in self.model.named_parameters():
            ...
            if any(x in k for x in freeze_layer_names):
                ...
                # LOGGER.info(f"WARNING ⚠️ setting 'requires_grad=False' for frozen layer '{k}'. "
                #             'See orgline.engine.trainer for customization of frozen layers.')
                v.requires_grad = False
        ...
        if self.amp and RANK in (-1, 0):  # Single-GPU and DDP
            ...
            # Keep as tensor instead of converting to boolean
            self.amp = torch.tensor(True, device=self.device)  # Instead of: self.amp = True
            callbacks.default_callbacks = callbacks_backup  # restore callbacks
        if RANK > -1 and world_size > 1:  # DDP
            dist.broadcast(self.amp, src=0)  # broadcast the tensor from rank 0 to all other ranks (returns None)
        self.amp = bool(self.amp)  # Convert to boolean AFTER broadcasting
        ...
        if self.args.cos_lr:
            ...
            self.scheduler = optim.lr_scheduler.StepLR(self.optimizer, step_size=self.args.lrf // self.args.lr0)
        else:
    ...
    def get_dataset(data):
        ...
        """
        return data['train'], data.get('val') or data.get('val')
    ...
    def validate(self):
        """
        Runs validation on val set using self.validator.
    ...
    def build_targets(self, preds, targets):
        """Builds target tensors for training ORGDET model."""
        pass
    ...
    def plot_training_samples(self, batch, ni):
        """Plots training samples during ORGDET training."""
        pass

    def plot_training_labels(self):
        """Plots training labels for ORGDET model."""
        pass
    ...
    def final_eval(self):
        """Performs final evaluation and validation for object detection ORGDET model."""
        ...
        if resume:
            try:
                ...
                raise FileNotFoundError('Resume checkpoint not found. Please pass a valid checkpoint to resume from, '
                                        "i.e. 'ORGDET train resume model=path/to/last.pt'") from e
        self.resume = resume

    def resume_training(self, ckpt):
        """Resume ORGDET training from given epoch and best fitness."""
        if ckpt is None:
            return
        best_fitness = 0.0
        ...
        if self.resume:
            assert start_epoch > 0, \
                ...
                f"Start a new training without resuming, i.e. 'ORGDET train model={self.args.model}'"
            LOGGER.info(
    ...
    def build_optimizer(self, model, model1,name='auto', lr=0.001, momentum=0.9, decay=1e-5, iterations=1e5):
        ...
        if name in ('Adam', 'Adamax', 'AdamW', 'NAdam', 'RAdam'):
            ...
            raise NotImplementedError(
                ...
                'To request support for addition optimizers please visit https://github.com/orgline/orgline.')
...
# from orgline.data.utils import check_cls_dataset, check_det_dataset
# from orgline.nn.tasks import attempt_load_one_weight, attempt_load_weights
# from orgline.utils import (DEFAULT_CFG, LOGGER, RANK, SETTINGS, TQDM_BAR_FORMAT, __version__, callbacks, clean_url,
#                                colorstr, emojis, yaml_save)
# from orgline.utils.autobatch import check_train_batch_size
# from orgline.utils.checks import check_amp, check_file, check_imgsz, print_args
# from orgline.utils.dist import ddp_cleanup, generate_ddp_command
# from orgline.utils.files import get_latest_run, increment_path
# from orgline.utils.torch_utils import (EarlyStopping, ModelEMA, de_parallel, init_seeds, one_cycle, select_device,
#                                            strip_optimizer)
#
#
...
#         self.plot_idx = [0, 1, 2]
#         # 初始化代码保持不变...
#         self.top_n_weights = []  # 用于存储最优的前5个权重文件
#         self.n_best = 5  # 定义要保存的最佳权重数量
#         # Callbacks
#         self.callbacks = _callbacks or callbacks.get_default_callbacks()
#         if RANK in (-1, 0):
#             callbacks.add_integration_callbacks(self)
...
#         # Add the current best model to top_n_weights
#         if len(self.top_n_weights) < self.n_best or self.fitness > self.top_n_weights[-1][1]:
#             # Insert new weight with fitness into the list and sort by fitness descending
#             self.top_n_weights.append((self.epoch, self.fitness))
#             self.top_n_weights = sorted(self.top_n_weights, key=lambda x: -x[1])
#
#             # If there are more than n_best weights, remove the worst one
#             if len(self.top_n_weights) > self.n_best:
#                 self.top_n_weights.pop(-1)
#
#             # Rename and save the top weights as top1.pt, top2.pt, ..., top5.pt
#             for idx, (epoch, fitness) in enumerate(self.top_n_weights):
#                 new_weight_path = self.wdir / f'top{idx + 1}.pt'
#                 torch.save(ckpt, new_weight_path, pickle_module=pickle)
#         if self.best_fitness == self.fitness:
...
#     # TODO: may need to put these following functions into callback
#     def plot_training_samples(self, batch, ni):
#         """Plots training samples during ORGDETv5 training."""
#         pass
#
#     def plot_training_labels(self):
#         """Plots training labels for ORGDET model."""
