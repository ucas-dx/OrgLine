...
from orgline.utils import LOGGER, TQDM, checks, clean_url, emojis, is_online, url2file

# Define orgline GitHub assets maintained at https://github.com/orgline/assets
GITHUB_ASSETS_REPO = 'orgline/assets'
GITHUB_ASSETS_NAMES = [f'ORGDETv8{k}{suffix}.pt' for k in 'nsmlx' for suffix in ('', '-cls', '-seg', '-pose')] + \
                      [f'ORGDETv5{k}{resolution}u.pt' for k in 'nsmlx' for resolution in ('', '6')] + \
                      [f'ORGDETv3{k}u.pt' for k in ('', '-spp', '-tiny')] + \
                      [f'ORGDET_nas_{k}.pt' for k in 'sml'] + \
...
def delete_dsstore(path, files_to_delete=('.DS_Store', '__MACOSX')):
    ...
    Example:
        ```python
        from orgline.utils.downloads import delete_dsstore
...
def zip_directory(directory, compress=True, exclude=('.DS_Store', '__MACOSX'), progress=True):
    ...
    Example:
        ```python
        from orgline.utils.downloads import zip_directory
...
def unzip_file(file, path=None, exclude=('.DS_Store', '__MACOSX'), exist_ok=False, progress=True):
    ...
    Example:
        ```python
        from orgline.utils.downloads import unzip_file
...
def check_disk_space(url='https://orgline.com/assets/coco128.zip', sf=1.5, hard=True):
    """
    Check if there is sufficient disk space to download and store a file.

    Args:
        url (str, optional): The URL to the file. Defaults to 'https://orgline.com/assets/coco128.zip'.
...
def get_google_drive_file_info(link):
    ...
    Example:
        ```python
        from orgline.utils.downloads import get_google_drive_file_info
...
def get_github_assets(repo='orgline/assets', version='latest', retry=False):
    """Return GitHub repo tag and assets (i.e. ['ORGDETv8n.pt', 'ORGDETv8s.pt', ...])."""
    if version != 'latest':
...
def attempt_download_asset(file, repo='orgline/assets', release='v0.0.0'):
    """
    Attempt file download from GitHub release assets if not found locally.

    release = 'latest', 'v6.2', etc.
    """
    from orgline.utils import SETTINGS  # scoped for circular import

    # ORGDETv3/5u updates
    file = str(file)
    file = checks.check_ORGDETv5u_filename(file)
