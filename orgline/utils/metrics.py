...
from orgline.utils import LOGGER, SimpleClass, TryExcept, plt_settings
...
def smooth_BCE(eps=0.1):
    ...
    For implementation details, refer to https://github.com/orgline/ORGDETv3/issues/238#issuecomment-598028441.
...
class ConfusionMatrix:
    ...
    def __init__(self, nc, conf=0.25, iou_thres=0.45, task='detect'):
        """Initialize attributes for the ORGDET model."""
        self.task = task
...
def plot_pr_curve(px, py, ap, save_dir=Path('pr_curve.png'), names=(), on_plot=None):
    ...
    data_path = save_dir.with_suffix('.pkl')

    # Save the data
    save_data = {'px': px, 'py': py, 'ap': ap}
    ...
    loaded_px, loaded_py, loaded_ap = loaded_data['px'], loaded_data['py'], loaded_data['ap']
    ...
    if 0 < len(names) < 21:  # display per-class legend if < 21 classes
        for i, y in enumerate(py.T):
            ax.plot(loaded_px, y, linewidth=1, label=f'{names[i]} {loaded_ap[i, 0]:.3f}')  # plot(recall, precision)
    else:
        ax.plot(loaded_px, py, linewidth=1, color='grey')  # plot(recall, precision)

    ax.plot(loaded_px, py.mean(1), linewidth=3, color='blue', label='all classes %.3f mAP@0.5' % loaded_ap[:, 0].mean())
...
def plot_mc_curve(px, py, save_dir=Path('mc_curve.png'), names=(), xlabel='Confidence', ylabel='Metric', on_plot=None):
    ...
    loaded_px, loaded_py = loaded_data['px'], loaded_data['py']
    fig, ax = plt.subplots(1, 1, figsize=(6, 6), tight_layout=True)

    if 0 < len(names) < 21:  # display per-class legend if < 21 classes
        for i, y in enumerate(loaded_py):
            ax.plot(loaded_px, y, linewidth=1, label=f'{names[i]}')  # plot(confidence, metric)
    else:
        ax.plot(loaded_px, loaded_py.T, linewidth=1, color='grey')  # plot(confidence, metric)

    y = smooth(loaded_py.mean(0), 0.05)
    ax.plot(loaded_px, y, linewidth=3, color='blue', label=f'all classes {y.max():.2f} at {px[y.argmax()]:.3f}')
    ...
    ax.legend(bbox_to_anchor=(0, 0), loc='lower left')
    ax.set_title(f'{ylabel}-Confidence Curve',fontsize=12)
    fig.savefig(save_dir, dpi=250)
    plt.close(fig)
    if on_plot:
        on_plot(save_dir)
...
class Metric(SimpleClass):
    """
    Class for computing evaluation metrics for ORGDETv8 model.
    ...
    def __init__(self) -> None:
        """Initializes a Metric instance for computing evaluation metrics for the ORGDETv8 model."""
        self.p = []  # (nc, )
        self.r = []  # (nc, )
        self.f1 = []  # (nc, )
        self.all_ap = []  # (nc, 10)
