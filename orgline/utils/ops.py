...
class Profile(contextlib.ContextDecorator):
    """
    ORGDETv8 Profile class. Use as a decorator with @Profile() or as a context manager with 'with Profile():'.
...
def scale_boxes(img1_shape, boxes, img0_shape, ratio_pad=None, padding=True):
    ...
    Args:
        ...
        padding (bool): If True, assuming the boxes is based on image augmented by ORGDET style. If False then do regular
            rescaling.
...
def non_max_suppression(
    ...
    Args:
        prediction (torch.Tensor): A tensor of shape (batch_size, num_classes + 4 + num_masks, num_boxes)
            ...
            output by a model, such as ORGDET.
    ...
    if isinstance(prediction, (list, tuple)):  # ORGDETv8 model in validation model, output = (inference_out, loss_out)
