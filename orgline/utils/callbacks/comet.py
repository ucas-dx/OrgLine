...
from orgline.utils import LOGGER, RANK, SETTINGS, TESTS_RUNNING, ops
...
try:
    ...
    # Names of plots created by ORGDETv8 that are logged to Comet
...
def _get_comet_model_name():
    """Returns the model name for Comet from the environment variable 'COMET_MODEL_NAME' or defaults to 'ORGDETv8'."""
    return os.getenv('COMET_MODEL_NAME', 'ORGDETv8')
...
def _create_experiment(args):
    ...
    try:
        ...
        experiment.log_other('Created from', 'ORGDETv8')
...
def _fetch_trainer_metadata(trainer):
    """Returns metadata for ORGDET training including epoch and asset saving status."""
...
def _scale_bounding_box_to_original_image_shape(box, resized_image_shape, original_image_shape, ratio_pad):
    """
    ORGDETv8 resizes images during training and the label values are normalized based on this resized shape.
...
def _format_prediction_annotations_for_detection(image_path, metadata, class_label_map=None):
    """Format ORGDET predictions for object detection visualization."""
...
def on_pretrain_routine_start(trainer):
    """Creates or resumes a CometML experiment at the start of a ORGDET pre-training routine."""
...
def on_fit_epoch_end(trainer):
    ...
    if curr_epoch == 1:
        from orgline.utils.torch_utils import model_info_for_loggers
