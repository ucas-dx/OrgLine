...
from orgline.hub.utils import HUB_WEB_ROOT, PREFIX, events
...
def on_model_save(trainer):
    """Saves checkpoints to orgline HUB with rate limiting."""
    session = getattr(trainer, 'hub_session', None)
    if session:
...
def on_train_end(trainer):
    """Upload final model and metrics to orgline HUB at the end of training."""
    session = getattr(trainer, 'hub_session', None)
    if session:
